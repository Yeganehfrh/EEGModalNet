...
class SimpleGAN(keras.Model):
    def __init__(self, time_dim=100, feature_dim=2, latent_dim=64, **kwargs):
        ...
        input_layer = keras.Input(shape=(self.latent_dim,))
        x = layers.Reshape((1, self.latent_dim))(input_layer)
        ...
        x = layers.LeakyReLU(negative_slope=0.2)(x)
        x = layers.Dense(self.time * self.feature)(x)
        x = layers.Reshape(self.input_shape)(x)

        # Residual connections
        x = layers.Add()([x, input_layer])

        self.generator = keras.Model(inputs=input_layer, outputs=x, name='generator')
